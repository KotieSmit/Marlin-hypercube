  ...
  float dx = RAW_X_POSITION(target[X_AXIS]),
    ...
    if (by_probe) {
      ...
      dy -= Y_PROBE_OFFSET_FROM_EXTRUDER;
            SERIAL_ECHO_START;
      SERIAL_PROTOCOLLN(dx);
      SERIAL_PROTOCOLLN(dy);
    }
  #endif
    ...
    const float dz = RAW_Z_POSITION(target[Z_AXIS]);
      SERIAL_PROTOCOLLN(dz);
      SERIAL_ECHOLNPGM("within dx: ");
      SERIAL_PROTOCOLLN(WITHIN(dx, X_MIN_POS - 0.0001, X_MAX_POS + 0.0001));

      SERIAL_ECHOLNPGM("within dy: ");
      SERIAL_PROTOCOLLN(WITHIN(dy, Y_MIN_POS - 0.0001, Y_MAX_POS + 0.0001));

      SERIAL_ECHOLNPGM("within dz: ");
      SERIAL_PROTOCOLLN(WITHIN(dz, Z_MIN_POS - 0.0001, Z_MAX_POS + 0.0001));
...
void set_current_from_steppers_for_axis(const AxisEnum axis) {
  ...
  inline bool prepare_kinematic_move_to(float ltarget[XYZE]) {
    ...
    if (ltarget[X_AXIS] == current_position[X_AXIS] && ltarget[Y_AXIS] == current_position[Y_AXIS]) {
        SERIAL_ECHOLNPGM(" marlin_main.cpp buffer_line_kinematic");  // _ks
